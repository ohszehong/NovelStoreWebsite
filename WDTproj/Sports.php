...
<div class="results">
      ...
      $sql = "SELECT * FROM lnovels WHERE ((LN_Name LIKE '%".$search_key."%') OR (LN_Author LIKE '%".$search_key."%') OR ((LN_Category_1 LIKE '%".$search_key."%') || (LN_Category_2 LIKE '%".$search_key."%'))) AND ((LN_Category_1 = 'Sports') || (LN_Category_2 = 'Sports'))";
      $result = mysqli_query($conn , $sql);

      if (mysqli_num_rows($result) <= 0)
      {
        echo "<script>alert('Books Not Found, Try other category!');</script>";
        $sql = "Select * from lnovels WHERE LN_Category_1 = 'Sports' || LN_Category_2 = 'Sports'";
        $result = mysqli_query($conn , $sql);
